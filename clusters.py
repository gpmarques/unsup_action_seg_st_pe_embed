...
class ClusterStrategy(ABC):
    ...
    def auto(self, data: np.ndarray) -> np.ndarray:
        ...
        for n in range(3, 96):
            ...
            if not_inc_iter >= 5 or data.shape[0] - 1 == n:
                break
...
class FINCHClusterStrategy(ClusterStrategy):
    ...
    def estimate(self, data: np.ndarray) -> np.ndarray:
        c, p_num_labels, _ = FINCH(data, distance=self.distance, verbose=False)
        prediction = c[:, len(p_num_labels) - 1]
        return prediction 
    
    def auto(self, data: np.ndarray) -> np.ndarray:
        return self.estimate(data)
    
class OPTICSClusterStrategy(ClusterStrategy):
    
    def __init__(self, min_samples: int = 2, min_sample_coverage: float = .05):
        self.min_samples = min_samples
        self.min_sample_coverage = min_sample_coverage
        self.set_estimator()
       
    def set_estimator(self):
        self.estimator = OPTICS(min_samples=self.min_samples, n_jobs=-1)
    
    def estimate(self, data: np.ndarray) -> np.ndarray:
        self.estimator.fit(data)
        return self.estimator.labels_
   
    def _sample_coverage_heuristic(self, data_len: int, fps: float, samples_frame_len: int):
        sample_coverage = samples_frame_len / data_len
        self.min_samples = math.ceil(self.min_sample_coverage / sample_coverage)
        self.set_estimator()
        
    def auto(self, data: np.ndarray, **kwargs) -> np.ndarray:
        self._sample_coverage_heuristic(data_len=data.shape[0], **kwargs)
        return self.estimate(data)
...
class ClusterFactory(Enum):
    ...
    def get(cluster_type: str) -> ClusterStrategy:
        ...
        return {
            ...
            ClusterFactory.FINCH.value: FINCHClusterStrategy,
            ClusterFactory.OPTICS.value: OPTICSClusterStrategy
        }[cluster_type]
